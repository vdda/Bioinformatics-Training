import os
import numpy as np
from pbcore.io.FastaIO import FastqEntry, writeFastqEntry
    ...
    for basFile, barcodeFile in zip(inputFofn, barcodeFofn):
        ...
        for hn in basH5.sequencingZmws[ msk ]:
            ...
            if zmw:
                ...
                if any(reads):
                    for read in reads:
                        outFastq.append(FastqEntry(read.readName, 
                                                   read.basecalls(),
                                                   read.QualityValue()))
  
    with open("%s/%s.fastq" % (outDir, outFile), 'w') as oFile:
        for e in outFastq:
            writeFastqEntry( oFile, e )
